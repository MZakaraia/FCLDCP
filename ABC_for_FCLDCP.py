...
def EmployedBeesFun(population, a, popSize, abandonmentVector, abandonmentLimit, Evals, BestEval, bestBee\
    ...
    for i in population:
        ...
        Employed.DistCenterArray = bestBee.DistCenterArray + np.random.uniform(-a, a, Employed.NumberOFDistCenter)*(Employed.DistCenterArray - rndBee.DistCenterArray)
        Employed.PlantArray = bestBee.PlantArray + np.random.uniform(-a, a, Employed.NumberOfPlants)*(Employed.PlantArray - rndBee.PlantArray)  
...
def OnlookerBees(population, a, popSize, abandonmentVector, abandonmentLimit, Evals, BestEval, bestBee\
    ...
    for i in population:
        ...
        Onlooker.DistCenterArray = bestBee.DistCenterArray + np.random.uniform(-a, a, Onlooker.NumberOFDistCenter)\
            *(Onlooker.DistCenterArray - Selected.DistCenterArray)
        Onlooker.PlantArray = bestBee.PlantArray + np.random.uniform(-a, a, Onlooker.NumberOfPlants)*(Onlooker.PlantArray - Selected.PlantArray)  
...
def ABC_Algorithm_FCLDCP(popSize, MaxItr, a, abandonmentLimit, Demands, Capacities, TransCosts, FixedCosts, membershipvalue = 0.6):
